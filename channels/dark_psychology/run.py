...
from channels.dark_psychology.scripts import get_script
from channels.dark_psychology.channel_config import (
...
def log_upload(title, video_id):
    with open(LOG_FILE, 'a') as f:
        f.write(f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M')} | DARK PSYCHOLOGY | {title} | https://youtube.com/watch?v={video_id}\n")
...
def run():
    print("\n" + "="*50)
    print("🧠 Dark Psychology Channel Pipeline")
    print("="*50 + "\n")
    try:
        audio_path  = f"{OUTPUT_FOLDER}/voiceover.mp3"
        bg_video    = "/home/ubuntu/ytbot/backgrounds/DarkPsychology.mp4"
        ...
        build_video(bg_video, audio_path, music_path, script, final_video, 'DARK PSYCHOLOGY')

        youtube = get_youtube_service(CLIENT_SECRETS, TOKEN_FILE)
        update_channel_description(youtube, "dark_psychology")

        description = f"""🧠 {hook}
{script}
Follow for daily dark psychology facts that could change your life.
📌 Also follow our Stoicism channel: https://www.youtube.com/channel/UCMWo8vpEpc4wv-8Jgnbxujg
#DarkPsychology #Psychology #MindControl #Facts #Shorts"""
        ...
        pin_comment(youtube, video_id, "dark_psychology")
        reply_comments(youtube)
        log_upload(title, video_id)
        cleanup()

        send_email(
            "✅ Dark Psychology — Video Uploaded!",
        ...
        print(f"❌ {msg}")
        send_email("❌ Dark Psychology Bot Error!", msg)
